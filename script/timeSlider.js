...
d3.csv(DATA_DIR + "covid_states_full_changes.csv", prepare).then(function (data) {
    d3.dsv(";", DATA_DIR + "election_polls.csv", prepare).then(function (polls) {
        d3.csv(DATA_DIR + "covid_counterMeasures.csv", prepare).then(function (event) {
...
function update(h) {
    ...
    //filters counterMeasures for the current month
    let newCounterM = counterM.filter(function (c) {

        //find out current slider-month
        let hMonth = h.getMonth();

        //find out counterMeasure-month
        let cMonth = c.date.getMonth();
